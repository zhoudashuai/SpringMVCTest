...
public class BillDaoImpl implements BillDao {
	...
	public List<Bill> getBillList(Connection connection,
			String queryproductName, int queryproviderId, int currentPageNo,
		...
		if (connection != null) {
			...
			/*
			 * if (bill.getIsPayment() > 0) { sql.append(" and isPayment = ?");
	...
	@Override
	public int getBillCountByProviderId(Connection connection,
			String productName, int providerId) throws Exception {
		// TODO Auto-generated method stub
		int count = 0;
		PreparedStatement pstm = null;
		ResultSet rs = null;
		if (connection != null) {
			...
			rs = BaseDao.execute(connection, pstm, rs, sql.toString(),
					list.toArray());
			if (rs.next()) {
				count = rs.getInt(1);
			}
